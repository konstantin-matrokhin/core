    ...
    public void execute(Channel channel) {
        ...
        if (Replies.hasCompanion(fromPlayer)) {
            ...
            if (Core.get().getOnlinePlayers().contains(toPlayer)) {
                String formatted = Localization.get(toPlayer, "pm-receive", from, message);
                MessagePacket msg = new MessagePacket(to, formatted);
                toPlayer.getCurrentProxy().send(msg);
                response = Localization.get(fromPlayer, "pm-response", to, message);
            } else {
                response = Localization.get(fromPlayer, "reply-no-player");
            }
        } else {
            response = Localization.get(fromPlayer, "reply-nobody");
        }
